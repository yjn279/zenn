...
1. 特徴量抽出：直近10本を読み、思考特性から記法までの各レイヤーで特徴を抽出する。抽出結果で `VOICE.md` を更新する。
2. 執筆：抽出した特徴を適用し、筆者本人が書いた記事と見分けがつかない状態を目指す。
3. 推敲：Composition・Visuals・Footnotes に照らして、章立て・図表の配分・出典を確認する。
