...
筆者の文体は `VOICE.md` に集約している。記事を1本書くたびに直近10本から取り直し、更新してから執筆に入る。過去に抽出した結果をそのまま使い回さない。記事が増えれば筆者の文体も動くためである。

以下の4段階を順に踏み、途中を飛ばさない。

1. 抽出：`articles/` の直近10本を読み、 `VOICE.md` の7つの観点で特徴を取り直す。思考特性に最も重きを置く。思考が揃えば構成や表現は結果として似てくるためで、表面の言い回しだけを真似ても筆者の記事にはならない。書きかけで止まっている下書きは見本にならないため除外する。
2. 執筆：`VOICE.md` を適用し、筆者本人が書いた記事と見分けがつかない状態を目指す。
3. 推敲：`VOICE.md` に照らして、章立て・図表の配分・語尾・出典を確認する。
4. 共有：プレビューを起動し、URL をユーザーへ渡す。

会話の中でユーザーが `VOICE.md` と異なる指示を出したときは、その場で該当する章へ反映する。記事を書き終えたときも、一連のやり取りを振り返って反映する。記事で裏の取れないルールは、次の抽出で書き換わって消える。
...
いずれも汎用のSkillであり、記事の文体については `VOICE.md` を優先する。筆者の実際の文体が正である。
...
プレビューの URL は `http://<取得したIP>:8000/articles/<スラッグ>` の形でユーザーへ渡す。同じ WiFi につながったスマートフォンから、リポジトリを開かずに仕上がりを確認できる。
