...
出発点は、はじめに書いた「視座が下がる」という課題そのものでした[^2]。
...
`/loop` は会話そのものを占有するため使えず、既存のフックはいずれも時間では発火しません。この時点で advisor ツールの存在を知っていれば、ここで終わっていたはずです。

では、時間で発火できないなら何をきっかけにすればいいのか。最初は `PostToolBatch`（ツールを使い終えた直後）で試作しましたが、`PostCompact`（会話が要約された直後）へ移すことで設計が変わりました。
...
役割は英語で与えていますが、答えは要約の言語に合わせて返ってきます。所要は 14 秒前後でした。実装はこの Pull Request にまとめてあります[^4]。
...
タグで囲むだけで、問いが出ない失敗はほぼ防げます。前置きが付くかどうかは別の観点で、減らせたのは最も具体的な `<coachee_work_summary>` だけでした。それでも、最終的にはシンプルさを取って `<summary>` を採用しました。
